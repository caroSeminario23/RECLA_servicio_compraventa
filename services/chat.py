...
def obtener_mensaje_entre_usuarios(id_usuario_1, id_usuario_2):
    ...
    if chat and chat.mensajes:
        logger.info(f"[Lógica interna] chat {chat.id} encontrado con {len(chat.mensajes)} mensajes.")
...
def agregar_mensaje(id_usuario_1, id_usuario_2, mensaje):
    ...
    try:
        if chat:
            ...
            logger.info(f"[Lógica interna] Agregando mensaje al chat existente: {chat.id}")
